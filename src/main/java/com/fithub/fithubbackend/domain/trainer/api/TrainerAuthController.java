...
public class TrainerAuthController {
    ...
    @Operation(summary = "트레이너 인증 요청, swagger에서는 careerList를 application/json으로 못 보내서 사용 X, postman에서는 Body->form-data로 요청 가능",
            description = "postman 요청 시 licenseFileList[0], careerList[0].company, specialtyList[0] 이런 식으로 보내야됨",
            responses = {
            ...
            @Parameter(name="requestDto", description = "트레이너 인증 요청 dto(자격증 이미지 파일 리스트, 자격증 이름, 경력사항 리스트, 전문 분야 리스트)")
    })
