...
public class UserTrainingReservationController {
    ...
    @GetMapping("/all")
    public ResponseEntity<Page<UsersReserveOutlineDto>> getUsersTrainingReservationList(@RequestParam(required = false) ReserveStatus status,
                                                                                        @AuthUser User user,
    ...
    @Operation(summary = "회원의 트레이닝 예약 내역 하나 상세 조회", parameters = {
            @Parameter(name = "reservationId", description = "조회할 예약 id")
    }, responses = {
            @ApiResponse(responseCode = "200", description = "조회 성공"),
            @ApiResponse(responseCode = "401", description = "로그인한 사용자만 가능", content = @Content(schema = @Schema(implementation = ErrorResponseDto.class)))
    })
    @GetMapping
    public ResponseEntity<UsersReserveInfoDto> getUsersTrainingReservation(@RequestParam Long reservationId,
                                                                           @AuthUser User user) {
        if(user == null) throw new CustomException(ErrorCode.AUTHENTICATION_ERROR, "로그인한 사용자만 가능합니다.");
        return ResponseEntity.ok(userTrainingReservationService.getTrainingReservation(reservationId));
    }
